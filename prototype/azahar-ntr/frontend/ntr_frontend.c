...
static void *guest_ptr(uint32_t addr,size_t size) {
    for(unsigned i=0;i<g_num_descs;i++){
        ...
        size_t t=(size_t)((a&~(uint64_t)d->disconnect)-d->start+d->offset);
        if(t+size>d->offset+d->len)continue;
        return(uint8_t*)d->ptr+t;
    }
    return NULL;
}
static bool env_cb(unsigned cmd,void *data) {
    ...
    case 15:if(data){struct retro_variable*v=data;if(!v->key)return false;
...
#define LOADSYM(name) do{*(void**)(&name)=dlsym(handle,#name);if(!name){fprintf(stderr,"missing %s: %s\n",#name,dlerror());dlclose(handle);return 2;}}while(0)
...
static int validate_summary(const uint32_t*s){
    ...
    fprintf(stderr,"===================================\n\n");
    if(s[0]!=GK_PASS||s[1]!=GK_EXPECTED_PROTOCOL||s[2]!=GK_EXPECTED_CAPS||s[3]!=GK_EXPECTED_LOCAL_RAM||
       s[4]==0||s[5]==0||s[6]!=GK_EXPECTED_OFFLOAD_US||s[7]!=GK_EXPECTED_SPEEDUP_X1000||s[8]!=GK_EXPECTED_CHECKSUM){
...
int main(int argc,char**argv){
    ...
    g_core_path=argv[1];const char*content=argv[2];const char*v;
    ...
    void*handle=dlopen(g_core_path,RTLD_NOW|RTLD_LOCAL);if(!handle){fprintf(stderr,"dlopen failed: %s\n",dlerror());return 2;}
    ...
    if(loaded){
        ...
        for(unsigned frame=0;frame<240;frame++){
            retro_run();uint32_t*s=(uint32_t*)guest_ptr(GK_SUMMARY_ADDR,9*sizeof(uint32_t));if(!s)continue;
            if(s[0]==GK_PASS){result=validate_summary(s);break;}
            if(s[0]==GK_FAIL){fprintf(stderr,"NTR E2E FAIL: guest reported failure\n");result=1;break;}
        }
